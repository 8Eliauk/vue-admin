...
export default {
  ...
  data() {
    return {
      ...
      attrInfo: {
        attrName: "", // 属性名
        attrValueList: [ // 属性值，因为属性值可以有多个，因此需要的是数组，每一个属性值都是一个对象需要attrId,valueName
  ...
  methods: {
    ...
    // 获取平台属性的数据
    // 当用户确认三级分类的数据时候，可以向服务器发请求获取平台属性进行展示
    async getAttrList() {
      // 解析数据 获取分类的ID
    ...
    addAttrValue() {
      ...
      this.attrInfo.attrValueList.push({
        attrId:this.attrInfo.id, // 对于修改某一个属性的时候，可以在已有的属性值基础之上新层新的属性值，（新增属性值的时候，需要把已有的属性的id带上）
        valueName:'',
    ...
    updateAttr(row) {
      ...
      this.attrInfo.attrValueList.forEach(item => {
        ...
        // 因为Vue无法探测普通的新增 property，这样书写的属性并非响应式属性（数据变化视图不变化）
        // item.flag = false;
        // 第一个参数：对象，第二个参数：添加新的响应式属性，第三个参数：新的属性的属性值
        this.$set(item,'flag',false);
      })
    },
    // 失去焦点的事件---切换为查看模式，战术是span
    toLook(row) {
      // 如果属性值为空不嫩作为新的属性值，需要给用户提示，让他输入一个其他的属性值
    ...
    async addOrUpdateAttr() {
      // 整理参数：1.添加很多属性值，且属性值为空的不应该提交给服务器
